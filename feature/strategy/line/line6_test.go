...
func TestLine6LongSignalBuysDip(t *testing.T) {
	defer withKlines([]*futures.Kline{k("100", "100", "99.6", "99.6", 1000)}, nil)()
	defer withRegime(0)()

	res := TradeLine6{}.GetCanLongOrShort(strategy.OpenParams{Symbols: &models.Symbols{Symbol: "TESTUSDT"}})
	if !res.CanLong {
		t.Fatalf("3分钟下跌0.4%%应触发做多(逆势), got %+v", res)
	}
	if res.CanShort {
		t.Fatal("下跌不应触发做空")
	}
...
func TestLine6ShortSignalFadesRip(t *testing.T) {
	defer withKlines([]*futures.Kline{k("100", "100.4", "100", "100.4", 1000)}, nil)()

	res := TradeLine6{}.GetCanLongOrShort(strategy.OpenParams{Symbols: &models.Symbols{Symbol: "TESTUSDT"}})
	if !res.CanShort {
		t.Fatalf("3分钟上涨0.4%%应触发做空(逆势), got %+v", res)
	}
}

func TestLine6ThresholdBoundary(t *testing.T) {
	// 0.4%(≥0.3%阈值): 触发 (99.6/100; 边界附近受浮点表示影响, 夹具取无歧义值)
	defer withKlines([]*futures.Kline{k("100", "100", "99.6", "99.6", 1000)}, nil)()
	res := TradeLine6{}.GetCanLongOrShort(strategy.OpenParams{Symbols: &models.Symbols{Symbol: "TESTUSDT"}})
	if !res.CanLong {
		t.Fatalf("0.4%%应触发, got %+v", res)
	}
	// 0.25%: 不触发
